...
import (
	...
	"github.com/tissazak/nowon/metrics"
	"github.com/tissazak/nowon/trie"
...
func (q *queue) reserveHeaders(p *peer, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	...
	for proc := 0; proc < space && len(send) < count && !taskQueue.Empty(); proc++ {
		...
		if index >= len(q.resultCache) || index < 0 {
			glog.Error("index allocation went beyond available resultCache space.\nYou've encountered a sought after, hard to reproduce bug. Please report this to the developers <3 https://github.com/tissazak/nowon/issues")
...
func (q *queue) DeliverNodeData(id string, data [][]byte, callback func(error, int)) (int, error) {
	...
	metrics.DLStateTimer.UpdateSince(request.Time)
